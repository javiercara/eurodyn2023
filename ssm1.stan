...
real kfilter_logL(vector y, matrix A, matrix C, matrix Q, 
  ...
  real Stinv;
  matrix[nx,ny] Kt;
  matrix[nx,nx] Inx = diag_matrix(rep_vector(1.0, nx));
...
real ssm1_lpdf(vector y, real f, real z, real dt, real H, 
  ...
  return kfilter_logL(y, A, C, Q, R, x10, P10);
...
model {
  ...
  // likelihood
  target += ssm1_lpdf(y | f, z, dt, H, E, m1, P1);
}
